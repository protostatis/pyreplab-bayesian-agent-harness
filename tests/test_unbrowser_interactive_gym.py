...
from pyreplab_harness.unbrowser_interactive_gym import (
    EXPECTED_RESULT,
    generate_unbrowser_interactive_task,
    verify_unbrowser_interactive_attempt,
)
from pyreplab_harness.unbrowser_rpc import UNBROWSER_INTERACTIVE_URL


class UnbrowserInteractiveGymTest(unittest.TestCase):
    def test_generation_is_deterministic_and_url_is_fixed(self) -> None:
        with tempfile.TemporaryDirectory() as first, tempfile.TemporaryDirectory() as second:
            task_a = generate_unbrowser_interactive_task(first, 7, "easy")
            task_b = generate_unbrowser_interactive_task(second, 7, "easy")
            ...
            self.assertNotIn(
                ...
                task_a.prompt.split("\n\n")[0],  # Only check the opening sentence
            )
            self.assertEqual(task_a.family, "unbrowser_interactive")

    def test_expected_heading_passes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            task = generate_unbrowser_interactive_task(directory, 7, "easy")
            attempt = prepare_attempt(directory, task.id, "ub-int-pass", "correct")
            write_json(Path(attempt.workspace_ref) / "result.json", EXPECTED_RESULT)
            result = verify_unbrowser_interactive_attempt(
    ...
    def test_wrong_heading_and_missing_output_fail(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            task = generate_unbrowser_interactive_task(directory, 7, "easy")
            wrong = prepare_attempt(directory, task.id, "ub-int-wrong", "wrong")
            write_json(
                Path(wrong.workspace_ref) / "result.json",
                {"heading": "Wikipedia Main Page"},
            )
            mismatch = verify_unbrowser_interactive_attempt(
            ...
            missing = prepare_attempt(directory, task.id, "ub-int-missing", "wrong")
            missing_result = verify_unbrowser_interactive_attempt(
